...
# Částka, kterou jsi ochoten ztratit na JEDEN OBCHOD (v USD/EUR)
RISK_NA_OBCHOD = 50 
...
def analyzuj_a_posli(symbol, nazev):
    ...
    # Kolik kusů si můžeš dovolit koupit?
    pocet_kusu = int(RISK_NA_OBCHOD / riziko_na_kus) if riziko_na_kus > 0 else 0
    
    # Odkaz na graf (TradingView)
    tv_symbol = symbol.replace("GC=F", "COMEX:GC1!").replace(".DE", "")
    chart_url = f"https://tradingview.com{tv_symbol}"

    zprava = (
        f"**{nazev}**\n"
        f"Trend: **{smer}** | RSI: `{rsi:.0f}`\n"
        f"💰 **DOPORUČENÝ OBJEM:** `{pocet_kusu} ks/oz` (při risku {RISK_NA_OBCHOD}$)\n"
        f"--- 💡 PLÁN ---\n"
